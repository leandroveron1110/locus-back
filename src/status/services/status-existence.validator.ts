...
@Injectable()
export class StatusExistenceValidator implements IExistenceValidator {
  constructor(private prisma: PrismaService) {}

  async checkOne(id: string): Promise<void> {
    const category = await this.prisma.status.count({
      where: { id },
    });
    if (!category) {
      throw new NotFoundException(`Status con ID "${id}" no encontrada.`);
    }
  }
  ...
  async checkMany(ids: string[]): Promise<void> {
    ...
    if (count !== uniqueIds.length) {
      ...
      const foundCategories = await this.prisma.status.findMany({
        where: { id: { in: uniqueIds } },
        select: { id: true }
      });
      const foundIds = new Set(foundCategories.map(c => c.id));
      const missingIds = uniqueIds.filter(id => !foundIds.has(id));
      throw new NotFoundException(`Algunos status no fueron encontradas: ${missingIds.join(', ')}`);
    }
  }
}